...
def four(arg1):
    loop_str= "55 72 86"
    sum_digits = [int(x) for x in loop_str if isnumeric()]
...
def five(input):
    ...
    field_names = ['owner', 'nameofFile','encrypted','filesize']

    data = [
        {'owner': 'bert', 'nameofFile': 'helloWorld.py', 'encrypted': 'True', 'filesize': '1447'}
        {'owner': 'bert', 'nameofFile': 'strings.py', 'encrypted': 'False', 'filesize': '1318'}
        {'owner': 'jeff', 'nameofFile': 'dice.py', 'encrypted': 'False', 'filesize': '1445'}
    ]
